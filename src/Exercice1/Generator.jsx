...
function Generator() {
  const [elementQuote, setElementQuote] = useState(
  ...
  const handleClick = () => {
    const indexQ = Math.floor(Math.random() * quoteDatabase.length);
    const element = quoteDatabase[indexQ];
    setElementQuote(element);
  };

  return (
    <div className="container">
      <h2 className="title-generator">Generatores</h2>
      <Quote message={elementQuote} />
      <InputQuote handleClick={handleClick} />
    </div>
  );
}
